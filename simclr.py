...
from utils.common_config import get_criterion, get_model, get_filter_model, get_train_dataset,\
...
def main():
    ...
    # Filter Model
    print(colored('Retrieve filter model', 'blue'))
    filter_model, translation_dict = get_filter_model(p, './outputs/simclr500/cifar-10/scan/model.pth.tar')
    print('Model is {}'.format(model.__class__.__name__))
    print('Model parameters: {:.2f}M'.format(sum(p.numel() for p in model.parameters()) / 1e6))
    print(filter_model)
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        # Train
        print('Train ...')
        simclr_train(train_dataloader, model, criterion, optimizer, epoch, filter_model, translation_dict)
        ...
        if (epoch + 1) % 50 == 0:
            checkpoint_path = checkpoint_path = '{}_{}.pth.tar'.format(p['pretext_checkpoint'].replace('.pth.tar', ''), epoch + 1)
            print('Checkpoint ...')
            torch.save({'optimizer': optimizer.state_dict(), 'model': model.state_dict(),
