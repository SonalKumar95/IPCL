...
def get_criterion(p):
    if p['criterion'] == 'simclr':
        from losses.losses import SimCLRLoss, SupConLoss
        # criterion = SimCLRLoss(**p['criterion_kwargs'])
        criterion = SupConLoss(**p['criterion_kwargs'])
...
def get_filter_model(p, model_path):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20']:
            from models.resnet_cifar import resnet18
    ...
    from models.models import ClusteringModel
    model = ClusteringModel(backbone, p['num_classes'], 1) # Note only support for single head
    ...
    if model_path is not None and os.path.exists(model_path):
        ...
        raise ValueError('Filter model path does not exist {}'.format(pretrain_path))

    # print(state.keys())
    translation_dict = {key: value for key, value in state['hungarian_match']}

    return model, translation_dict
