...
def simclr_train(train_loader, model, criterion, optimizer, epoch, filter_model, translation_dict):
    """ 
    ...
    for i, batch in enumerate(train_loader):
        ...
        labels = torch.argmax(filter_model(images.cuda(non_blocking=True))[0], dim=1)
        labels = torch.tensor([translation_dict[label.item()] for label in labels]).cuda(non_blocking=True)
        # accuracy = torch.sum(labels == targets).item() / len(labels)
        # print(f'Accuracy: {accuracy * 100:.2f}%')
        theta = 0.95
        loss = theta * criterion(output) + (1 - theta) * criterion(output, labels) 
